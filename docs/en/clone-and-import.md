...
`git clone https://github.com/ByteLegendQuest/java-refactor-to-base-class`

A project directory named `java-refactor-to-base-class` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-refactor-to-base-class` directory which is created just now.
